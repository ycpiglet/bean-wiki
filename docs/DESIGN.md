...
| `--green-bean` | 에티오피아 예가체프 | `#7d8f69` | `#9db184` | 생두 단계·긍정적 진행 상태 |
| `--parchment` | 드라이 파치먼트 베이지 | `#e3d4b4` | `#d9c8a6` | 옅은 라이프사이클 배경 |
| `--honey` | 옐로 허니 파치먼트 | `#c8a24c` | `#ddbf73` | 허니 프로세스·성취 강조 |
| `--kenya-aa` | 케냐 AA | `#7e855a` | `#aab17d` | 공개 산지 팔레트(AA는 색이 아닌 크기·밀도 등급) |
| `--yellow-bourbon` | 브라질 옐로 버번 | `#c79b35` | `#ddb75e` | 공개 산지 팔레트(성숙 체리 장면) |
| `--wet-hulled` | 인도네시아 만델링 | `#5f796c` | `#8eaa9c` | 공개 산지 팔레트(습식 탈곡 생두 장면) |
| `--geisha-silver` | 파나마 게이샤 | `#a7aa91` | `#c4c8ad` | 공개 산지 팔레트(공식 품종색 아님) |
| `--monsooned-gold` | 인도 몬순드 말라바 | `#c3a66a` | `#dbc28d` | 공개 산지 팔레트(몬순 처리 생두 장면) |
| `--coffee-chaff` | 커피 채프 토프 | `#a39383` | `#c2b5a7` | 로스팅 부산물 팔레트 |
| `--roast-light` | 퍼스트 크랙 캐러멜 | `#a26a42` | `#c08a5c` | 로스팅 그라데이션의 밝은 지점(Agtron ~85) |
| `--roast-city` | 라이트 로스트 브라운 | `#8f5d3b` | `#b9825a` | 공개 로스팅 팔레트 |
| `--roast-medium` | 미디엄 로스트 브라운 | `#7c5233` | `#c89e6f` | UI 프라이머리(Agtron ~55) |
| `--roast-dark` | 다크 로스트 브라운 | `#4e3524` | `#8d6844` | 호버·다크 패널 그라데이션(Agtron ~35) |
| `--espresso` | 바닐라빈 블랙 | `#2b1d13` | `#3a2a1c` | 원두 마크·상시 다크 패널 |
| `--brew-amber` | 93° 브루잉 앰버 | `#b47a3b` | `#d19b68` | 공개 잔 팔레트 |
| `--crema` | 크레마 골드 | `#c99b5f` | `#d9b586` | 다크 패널 강조·원두 크리즈 |
...
| `--affogato` | 아포가토 크림 | `#d6b88b` | `#e2c9a5` | 공개 잔 팔레트 |
...
- 일곱 파트는 **각 6색**을 기본으로 하며, 하네스 허용 범위는 5–7색입니다.
- 한국어 표시명은 공백 포함 **12자 목표, 14자 상한**입니다.
- 짧은 한국어 제목에 생략된 색상군과 맥락은 바로 아래 영문명과 이야기에서
  보완합니다.
...
| `--ink` | 포터필터 블랙 | `#1e1b18` | `#ece7da` | 기본 텍스트·제목 |
...
|---|---|---|---|
| `--olive` | 로스터 에이프런 그린 | 커피 기초 · 커피와 건강 | `callout-tip` |
| `--sage` | 커피팜 세이지 | 산지와 생두 · 지속가능성과 거래 | — |
| `--copper` | 카퍼 체즈베 | 로스팅 · 커피 역사와 문화 | `callout-warn` |
| `--blue` | 워싱 탱크 블루 | 추출 · 커피 과학 | `callout-note` |
