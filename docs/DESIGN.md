...
인증을 뜻하지 않습니다. 공개용 영문명과 스토리는
`src/app/design/colors/page.tsx`가 정본이며, 아래 표는 한국어 표시명과 내부
기능 역할을 관리합니다.

| 토큰 | 표시용 정식 브랜드명 | 라이트 | 다크 | 기능 역할 |
|---|---|---|---|---|
| `--cherry` | 케냐 커피체리 레드 | `#a03d36` | `#cd7a6e` | 중요 상태·오답 강조 |
| `--cherry-deep` | 내추럴 드라이 체리 | `#712a28` | `#a75950` | 체리 계열의 깊은 보조 톤 |
| `--green-bean` | 에티오피아 워시드 그린빈 | `#7d8f69` | `#9db184` | 생두 단계·긍정적 진행 상태 |
| `--parchment` | 르완다 파치먼트 베이지 | `#e3d4b4` | `#d9c8a6` | 옅은 라이프사이클 배경 |
| `--honey` | 코스타리카 허니 파치먼트 | `#c08a3e` | `#d9a95e` | 허니 프로세스·성취 강조 |
| `--yellow-bourbon` | 브라질 옐로 버번 | `#c79b35` | `#ddb75e` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--pink-bourbon` | 콜롬비아 핑크 버번 | `#b76e79` | `#d69aa3` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--wet-hulled` | 수마트라 웻헐 제이드 | `#5f796c` | `#8eaa9c` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--geisha-silver` | 파나마 게이샤 실버그린 | `#a7aa91` | `#c4c8ad` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--monsooned-gold` | 인도 몬순드 말라바 골드 | `#c3a66a` | `#dbc28d` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--peaberry-olive` | 피베리 올리브 | `#7e855a` | `#aab17d` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--decaf-green` | 디카페인 그린빈 토프 | `#8a836c` | `#b3aa8d` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
...
| `--pink-salt` | 히말라얀 핑크 솔트 | `#d1a09a` | `#e5bcb6` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--hibiscus` | 히비스커스 티 루비 | `#9f3e5a` | `#cf728c` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--terroir-earth` | 볼캐닉 테루아 어스 | `#654638` | `#a07868` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--pitcher-silver` | 스테인리스 피처 실버 | `#b5b9b5` | `#d3d6d1` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
| `--terrazzo` | 카페 테라조 스톤 | `#aaa394` | `#c9c2b5` | 브랜드 스토리 팔레트(의미 상태 사용 금지) |
...
| 토큰 | 표시용 정식 브랜드명 | 라이트 | 다크 | 기능 역할 |
|---|---|---|---|---|
| `--paper` | 각설탕 크리스털 화이트 | `#f4f0e7` | `#14170f` | 페이지 배경 |
| `--paper-deep` | 오트 비스킷 베이지 | `#e8e1d4` | `#1b1f16` | 살짝 눌린 면(카드 등) |
| `--cream` | 바닐라 크림 | `#fbf8f1` | `#1e2318` | 입력·표면(에디터 등) |
| `--ink` | 에스프레소 마키아토 블랙 | `#182019` | `#ece7da` | 기본 텍스트·제목 |
| `--muted` | 쿠키 앤 크림 그레이 | `#62675f` | `#a0a394` | 보조 텍스트·라벨 |
...
| `--sage` | 레인포레스트 미스트 | 산지와 생두 · 지속가능성과 거래 | — |
| `--copper` | 웜 드럼 코퍼 | 로스팅 · 커피 역사와 문화 | `callout-warn` |
| `--blue` | 워시드 탱크 블루 | 추출 · 커피 과학 | `callout-note` |
| `--berry` | 블랙커런트 커핑 베리 | 센서리 · 음료와 레시피 | `callout-important` |
| `--sand` | 화이트 오크 카운터 | 카페와 장비 · 카페 운영과 품질 | — |
