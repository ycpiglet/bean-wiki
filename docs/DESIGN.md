...
- 아티클의 누적 조회·좋아요·댓글 수는 클릭 가능한 버튼과 시각적으로 분리한
  중립적 `문서 반응` 영역에 표시합니다. 수치는 버튼 안에서 반복하지 않으며
  로딩 중에는 실제 0으로 오해할 수 있는 값 대신 대시를 표시합니다.
- 좋아요·댓글·스크랩·공유·더 보기 도구는 본문과 문서 퀴즈를 모두 읽은 뒤,
  댓글 대화가 시작되기 직전에 한 번만 배치합니다. 제목 아래에는 읽기 시간,
  수정일, 편집·역사 같은 문서 메타데이터만 남깁니다.
- 아티클 도구는 독자의 의도에 따라 `좋아요·댓글` → `스크랩·공유` →
  `내보내기·인쇄` 순으로 계층화합니다. 내보내기와 인쇄처럼 사용 빈도가 낮은
  도구는 `더 보기` 메뉴에 두며 모든 기능을 같은 크기의 칸으로 나열하지 않습니다.
- 조회수는 상태 정보이므로 버튼처럼 강조하지 않습니다. 댓글 영역에서 조회수와
  좋아요 수를 다시 반복하지 않고, 댓글 수와 평균 평점만 대화 맥락에 표시합니다.
- 평점과 댓글은 별개의 큰 폼으로 나란히 배치하지 않습니다. 하나의 수직 작성기에서
  댓글만, 별점만, 또는 둘 다 남길 수 있고 별점은 항상 선택 사항입니다.
- 댓글, 답글, 수정 폼은 문서 읽기 순서를 유지하는 한 열 흐름을 사용합니다.
  대댓글은 한 단계만 들여쓰며 모바일에서는 들여쓰기를 줄여 본문 폭을 보존합니다.
- 주요 버튼과 별점 선택 표적은 최소 44px, 보조 댓글 동작은 WCAG 2.2 AA의
  최소 표적과 간격을 충족합니다. 키보드 포커스는 2px 브랜드색 외곽선으로
  텍스트·배경 변화와 함께 표시합니다.
- 카드 제목이 링크가 되어도 제목의 타입 역할을 상속해야 합니다. 본문·푸터
  링크의 작은 글자 규칙은 직접 자식이나 전용 클래스에만 적용하며 제목 내부
  링크를 포괄하는 후손 선택자로 덮어쓰지 않습니다.
